...
### Fixed
- `wb_modbus_add_devices` теперь читает шаблон каждого добавляемого устройства и
  копирует default-значения параметров (`device.parameters[].default`) в
  device-record. Без этого schema-валидация `wb-mqtt-serial` падала на
  required-параметрах (типичный кейс — WB-MAI6 `in1_type..in6_type`), и весь
  конфиг отвергался → опрос всей шины не запускался. В отчёте `added[]`
  появилось поле `paramDefaults` (число скопированных параметров).
