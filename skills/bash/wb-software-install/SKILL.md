...
- The backup (see `/wb-controller-backup`) picks up compose files and named volumes itself.
...
`/wb-controller-backup` picks up `docker-compose.yml`, `.env`, `data/` entirely; named volumes — separately via `docker run -v <vol>:/data alpine tar`.
...
1. **Verify the module** — a slot with a Zigbee adapter is required (see `/wb-hardware-modules`). Without the module Z2M won't start.
2. **Install:**
   ```bash
