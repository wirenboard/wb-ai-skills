...
**Don't confuse with `/troubleshooting`** (general "something broke" diagnostics). This skill is for targeted setup.
...
`connections` — ordered list of `connection_uuid` from highest to lowest priority. Failover follows it.

Editing via `confed/Editor/Save` (see `/wb-mqtt-serial` for the general pattern — same format).
...
- **NM doesn't start** — `systemctl status NetworkManager`, kernel mismatch (see `/troubleshooting`).
- **Custom nmconnection won't survive FIT** — backup via `/controller-backup`.
