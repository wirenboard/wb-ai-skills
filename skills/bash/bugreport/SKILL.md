...
Collect as much as possible yourself via the `/troubleshooting`, `/troubleshooting-serial`, `/wiren-board` skills and the mDNS warm-up from the master skill; ask as little as possible. The list in step 4 is a **fallback** for cases when self-diagnostics didn't yield an answer for a specific item.

**Before changing anything** on a production controller to test a hypothesis — `/controller-backup` (a short snapshot via step 1b).
...
Kernel mismatch (a frequent cause of issues after upgrade — see `/troubleshooting`):
```bash
...
This list is a fallback, for items not resolved by step 1. First check yourself via the skills (`/wiren-board`, `/troubleshooting`).
...
- `/troubleshooting` — general diagnostics (kernel mismatch, disk, failed units, error journal) — part of step 1 duplicates its commands; if you dig deeper, jump straight there.
- `/troubleshooting-serial` — if the symptom is about Modbus/RS-485 (CRC, timeout) — that's where the debug session lives, no `wb-diag-collect` needed.
- `/controller-backup` — mandatory safety snapshot before any changes to `/etc/*` for hypothesis testing.
- `/controller-update` — if the hypothesis is "a fresh package would fix it", check via recon from `/controller-update` (without running an actual upgrade).
