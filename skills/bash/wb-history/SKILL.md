...
```
If `inactive` — **don't install it yourself**, report to the user and agree: the package needs to be installed via `apt install wb-mqtt-db`, but this changes controller state and should go through the `/wb-software-install` skill (with disk space recon and a background job).
