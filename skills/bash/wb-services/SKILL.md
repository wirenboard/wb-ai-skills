---
name: wb-services
description: Managing systemd services and timers on a Wiren Board controller. Creating override-conf, drop-ins, custom units and timers. Enable, disable, mask. NOT for diagnostics (for that use `/wb-troubleshooting`).
...
ssh root@<HOST> 'journalctl -u <unit> -n 50 --no-pager'   # also see `/wb-troubleshooting`
```
...
**Custom units in `/etc/systemd/system/`** do not survive a FIT flash (it overwrites rootfs). For backup — see `/wb-controller-backup`, "Custom systemd units" section.
