...
2. Make a backup: `/wb-controller-backup`
...
1. **Backup is mandatory**: invoke `/wb-controller-backup` fully, download the archive locally. After reset you can't restore anything without it. **`/mnt/data/` is wiped completely** — including `/mnt/data/ai/` (snapshots, diag logs, backups). Download everything needed locally before the reset.
...
6. **Restore** — via `RESTORE.md` from the backup (see `/wb-controller-backup` restore scenario).
