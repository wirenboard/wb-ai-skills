...
```bash
ssh root@<HOST> wb-cli modbus templates                  # list all template filenames
ssh root@<HOST> wb-cli modbus template wb-mr6c           # show full JSON of a template
```
...
(`-t 4` = `input registers`, FC4. See `/wb-troubleshooting-serial`.)
...
A custom template won't survive FIT — must go into `/wb-controller-backup` (it picks up `/etc/wb-mqtt-serial.conf.d/` itself).
