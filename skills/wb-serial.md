...
## Adding a device to wb-mqtt-serial via confed

To add a device without editing JSON files directly, use the confed API:

```bash
# Read current config
ssh root@<HOST> wb-cli --json confed load /etc/wb-mqtt-serial.conf
```

The output is `{"data": {...}}`. Extract `.data`, add your device under `.config.ports[N].devices[]` following the schema (must have at least `slave_id` and `device_type`), then save:

```bash
ssh root@<HOST> 'wb-cli --json confed save /etc/wb-mqtt-serial.conf '"'"'<updated-json>'"'"''
```

After save, `wb-mqtt-serial` reloads automatically. Verify: `wb-cli --json dev <device_id>`.
