...
**Before changing anything** on a production controller to test a hypothesis — `wb_state_save` (short snapshot, see `/wb-controller-backup`).
...
- `/wb-troubleshooting` — general diagnostics (kernel mismatch, disk, failed units).
- `/wb-troubleshooting-serial` — if symptom involves Modbus/RS-485 (debug session + scan, not a diag archive).
- `/wb-controller-backup` — safety snapshot before any changes to `/etc/*`.
- `/wb-controller-update` — recon without upgrade, to check "would a fresh package fix it".
