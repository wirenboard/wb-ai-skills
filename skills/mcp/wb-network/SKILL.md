...
- **NM profiles in `/etc/NetworkManager/system-connections/*.nmconnection` won't survive FIT** — backup via `/wb-controller-backup`.

## Related skills

- `/wb-troubleshooting` — general "doesn't work" diagnostics.
- `/wb-services` — custom `*.service` for running VPN/scripts.
- `/wb-controller-backup` — saving NM profiles before FIT.
- `/wb-cloud` — cloud agent over the internet.

Details (nmcli syntax, OpenVPN, mmcli) — bash-flavor twin `/wb-network`.
