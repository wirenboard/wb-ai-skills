...
| Backup before release switch | skill `/wb-controller-backup` |
...
**Kernel mismatch before upgrade** = reboot first, then redo recon (see `/wb-troubleshooting`).
...
2. Make a backup: invoke `/wb-controller-backup`.
...
1. **Backup is mandatory** (invoke `/wb-controller-backup` in full, download locally):
   ```
   /controller-backup
   ```
