...
```bash
ssh root@<HOST> wb-cli confed load /etc/wb-scenarios.conf
```

### Save updated config

```bash
ssh root@<HOST> 'wb-cli confed save /etc/wb-scenarios.conf '"'"'{"scenarios":[...]}'"'"''
```

`content` is a **JSON object**, not a string (see `/wb-mqtt-serial`, same format). `wb-cli confed save` wraps the confed RPC — no need for manual `mosquitto_sub`/`mosquitto_pub` with CID.
